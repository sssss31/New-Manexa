...
export async function POST(req: Request) {
  ...
  const invoice = await prisma.invoice.findFirst({
    ...
    include: { tenant: { select: { name: true } }, student: { include: { user: true } }, payments: { select: { amount: true } } },
  });
  if (!invoice) return Response.json({ error: "Invoice not found" }, { status: 404 });
  if (invoice.status === "PAID") return Response.json({ error: "This invoice is already paid" }, { status: 409 });

  // Charge the OUTSTANDING balance (total − already paid), so a partially-paid
  // invoice is never over-charged the full amount again.
  const paid = invoice.payments.reduce((s, p) => s + p.amount, 0);
  const outstanding = invoice.total - paid;
  if (outstanding <= 0) return Response.json({ error: "This invoice is already paid" }, { status: 409 });

  try {
    const order = await createRazorpayOrder(outstanding * 100, invoice.number, {
