...
export async function payInvoiceAction(formData: FormData) {
  const a = await actor("fee.manage");
  // Optional partial amount — blank/absent means pay the full outstanding balance.
  const raw = String(formData.get("amount") ?? "").trim();
  const amount = raw ? Math.round(Number(raw)) : undefined;
  if (raw && (!Number.isFinite(amount!) || amount! <= 0)) throw new Error("Enter a valid payment amount");
  await payInvoice({
    tenantId: a.tenantId!,
