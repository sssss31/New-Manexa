...
export default async function InstitutionCockpit() {
  ...
  return (
    <div className="relative">
      ...
      <div className="dash-orb dash-orb-dim w-[380px] h-[380px] top-40 right-[-120px]" aria-hidden />
      ...
      <Reveal className="relative z-[1] flex flex-wrap items-end justify-between gap-4 mb-7">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/institution/notices" className="btn-secondary gap-1.5"><Plus size={16} /> New notice</Link>
        </div>
      </Reveal>
      ...
      <Reveal className="relative z-[1] grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        <Panel title="Attendance trend" icon={<TrendingUp size={16} />} right="last 10 days · %" className="lg:col-span-2">
          {attData.length > 1 ? (
        ...
        <Panel title="Fee mix" icon={<Wallet size={16} />} right="₹ thousands">
      ...
      <Reveal className="relative z-[1] mb-4">
        <Panel title="Class strength" icon={<Users size={16} />} right="students per class">
      ...
      <div className="relative z-[1] grid grid-cols-1 lg:grid-cols-3 gap-4">
        <Reveal className="lg:col-span-2">
          <Panel title="Upcoming exams" icon={<ClipboardList size={16} />} right={<Link href="/institution/exams" className="text-accent hover:underline">All exams</Link>}>
            {upcomingExams.length === 0 ? (
              <EmptyMini icon={<ClipboardList size={22} />} text="No exams scheduled." />
            ) : (
              <ul className="space-y-1">
                {upcomingExams.map((e) => (
                  <li key={e.id} className="flex items-center justify-between rounded-xl px-3 py-2.5 hover:bg-fg/[0.03] transition-colors">
                    <div>
        ...
        <Reveal delay={0.05}>
          <Panel title="Recent notices" icon={<BellRing size={16} />} right={<Link href="/institution/notices" className="text-accent hover:underline">All</Link>}>
            {recentNotices.length === 0 ? (
              <EmptyMini icon={<Megaphone size={22} />} text="No notices yet." />
            ) : (
              <ul className="space-y-2.5">
                {recentNotices.map((n) => (
                  <li key={n.id} className="pb-2.5 border-b border-fg/8 last:border-0 last:pb-0">
      ...
      <div className="relative z-[1] grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4">
        <Reveal>
          <Panel title="AI insight" icon={<Sparkles size={16} />}>
            <div className="flex flex-col h-full">
              ...
              <Link href="/institution/ai" className="btn-primary w-full justify-center mt-4 gap-1.5">
                Open AI Insights <ArrowUpRight size={16} />
              </Link>
            </div>
          </Panel>
        </Reveal>
        ...
        <Reveal delay={0.05} className="lg:col-span-2">
          <Panel title="Recent activity" icon={<Activity size={16} />} right={<Link href="/institution/audit" className="text-accent hover:underline">Audit log</Link>}>
            {recentAudit.length === 0 ? (
              <EmptyMini icon={<Activity size={22} />} text="No activity yet." />
            ) : (
              <ul className="space-y-0.5">
                {recentAudit.map((a) => (
                  <li key={a.id} className="flex items-baseline justify-between gap-3 rounded-lg px-2 py-1.5 hover:bg-fg/[0.03] transition-colors">
...
function EmptyMini({ icon, text }: { icon: React.ReactNode; text: string }) {
  return (
    <div className="flex flex-col items-center justify-center gap-2 py-8 text-center">
      <span className="w-11 h-11 rounded-xl bg-fg/[0.04] border border-fg/8 flex items-center justify-center text-subtle">{icon}</span>
