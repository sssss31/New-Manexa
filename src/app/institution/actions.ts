...
export async function createEventAction(formData: FormData) {
  ...
  const CATS = ["EVENT", "HOLIDAY", "MEETING", "ADMISSION", "ACTIVITY", "TRAINING"];
  const rawCat = String(formData.get("category") || "EVENT").toUpperCase();
  const category = CATS.includes(rawCat) ? rawCat : "EVENT";
  ...
  revalidatePath("/institution/calendar");
}
...
// ---- Staff leave review (HR / admin) ----
export async function reviewLeaveAction(formData: FormData) {
  const a = await actor("staff.manage");
  const { reviewLeave } = await import("@/lib/leave");
  await reviewLeave({
    tenantId: a.tenantId!, actorId: a.id,
    leaveId: String(formData.get("leaveId")),
