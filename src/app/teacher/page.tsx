...
export default async function TeacherHome() {
  ...
  // The teacher's OWN attendance for today (face self check-in writes this row).
  const myAttendance = staff
    ? await prisma.staffAttendance.findUnique({
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<Link href="/teacher/attendance" className="btn-secondary">Class attendance</Link>}
      />
      ...
      {/* My attendance today — the teacher's own face self check-in status. */}
      <SectionCard className="mb-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <span className={`w-2.5 h-2.5 rounded-full ${myAttendance ? (myAttendance.status === "LATE" ? "bg-warning" : "bg-success") : "bg-subtle"}`} />
            <div>
              <div className="text-sm font-medium text-fg">My attendance today</div>
              <div className="text-xs text-muted">
                {myAttendance
                  ? `Marked ${myAttendance.status.toLowerCase()} at ${(myAttendance.firstInAt ?? new Date()).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit", timeZone: "Asia/Kolkata" })} · Face`
                  : "Not marked yet"}
              </div>
            </div>
          </div>
          {myAttendance ? (
            ...
            <Link href="/teacher/attendance/self" className="btn-primary text-sm">Check in with Face</Link>
          )}
        </div>
      </SectionCard>
