...
export function FaceCapture({
  ...
  subjectId: string;
  subjectName: string;
  /** Hide raw quality metrics + jargon — for self-service "Face ID" setup. */
  simple?: boolean;
  /** Fired once every required pose has been captured. */
  onComplete?: () => void;
}) {
  ...
  // Quality drives the live guidance (ring colour + hint text), but the button
  // is enabled as soon as the camera is live — a disabled button with no clear
  // reason is a dead end. If a frame is genuinely too dark/blurry, capture()
  // still rejects it with a specific, actionable message.
  const qualityOk = camReady && live.reasons.length === 0;
  const canCapture = camReady && !busy;

  // Signal the parent flow once every required pose is captured.
  useEffect(() => {
    if (captured === ENROLL_POSES.length) onComplete?.();
  }, [captured, onComplete]);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="card p-4">
        <div className="relative rounded-xl overflow-hidden bg-black aspect-[4/3]">
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div
              ...
              style={{
                width: "58%", height: "78%",
                borderColor: qualityOk ? "rgb(var(--accent))" : "rgb(var(--border))",
              }}
            />
          </div>
          {camError && (
        ...
        >
          {busy ? "Hold still…" : !camReady ? "Starting camera…" : simple ? "Capture" : `Capture ${activePose.replace(/(\d+)/, " $1°")}`}
        </button>
        {camReady && !qualityOk && !busy && (
          <p className="text-xs text-muted text-center mt-1.5">{live.reasons[0] ?? "Adjust position and lighting"} — you can still try capturing.</p>
        )}
        {toast && (
