// Inline icon set — lucide-style paths — keeps the app dep-lean.
// Default size is the design-system `base` token (18px): a sensible floor so an
// icon rendered with no explicit size never looks lost. Pass `size` (px) to
// override per container per the iconSize tokens in @/lib/design-system.

import { iconSize } from "@/lib/design-system/icons";

type P = { size?: number };
const S = ({ children, size = iconSize.base }: { children: any; size?: number }) => (
