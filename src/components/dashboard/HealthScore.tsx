    ...
    >
      <div className="grid gap-6 md:grid-cols-[auto_1fr] items-center">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <span className={`badge ${band.text} bg-fg/[0.04] border-fg/10`}>{band.label}</span>
        </div>
      ...
      {health.suggestions.length > 0 && (
        <div className="mt-5 pt-4 border-t border-fg/8">
            ...
            {health.suggestions.map((s, i) => {
              ...
              const body = (
                <div className="flex items-start gap-2.5">
                  ...
                  {s.href && <ArrowUpRight size={16} className="mt-0.5 ml-auto shrink-0 text-subtle" />}
                </div>
              );
              return (
                <li key={i} className="rounded-lg px-2 py-1.5 hover:bg-fg/[0.03] transition-colors">
