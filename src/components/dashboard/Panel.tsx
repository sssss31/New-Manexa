...
export function Panel({
  ...
  return (
    <section className={cn("glass-panel relative z-[1]", className)}>
      {(title || right) && (
        <header className="flex items-center justify-between gap-3 px-5 py-3.5 border-b border-fg/8">
          <div className="flex items-center gap-2">
            {icon && <span className="text-accent inline-flex items-center [&>svg]:w-[18px] [&>svg]:h-[18px]">{icon}</span>}
