...
import { postExpenseJournal } from "./accounting";
...
export async function createExpense(tenantId: string, actorId: string | null, data: NewExpense) {
  ...
  // Post the double-entry journal (Dr Expense account, Cr Cash/Bank).
  await postExpenseJournal({
    tenantId, expenseId: expense.id, amount, category, paidVia,
    description, date: expense.spentAt, actorId,
  }).catch(() => {});
