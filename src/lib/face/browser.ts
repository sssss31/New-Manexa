...
export async function analyzeFrame(video: HTMLVideoElement): Promise<Analysis> {
  ...
  // The Chromium Shape Detection API is flaky: it's often absent, and when
  // present it frequently returns 0 boxes for a face that's clearly there. Both
  // cases must NOT hard-block capture, so we fall back to a centered crop and
  // assume a single face is present whenever the detector gives us nothing. When
  // it DOES return boxes we trust the real count (so multi-face is still caught).
  ...
  return {
    quality: {
      brightness,
      sharpness,
      // Assume one face when the detector gives us nothing (unsupported OR a
      // flaky empty result); use the real count only when it actually detected.
      // The size/brightness/sharpness gates still apply on the cropped region.
      faceCount: detected ? boxes.length : 1,
      faceBoxPx,
      centered,
      frameW: w,
      frameH: h,
