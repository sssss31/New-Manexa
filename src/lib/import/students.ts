...
import { assignManexaId } from "@/lib/manexa-id";
...
export interface ImportResult {
  ...
  // Per-row outcome for the result table: Name | Type | MANEXA ID | Status.
...
export async function commitStudents(
  ...
  const result: ImportResult = { imported: 0, skipped: 0, failed: 0, createdClasses: [], failures: [], created: [] };
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      const manexaId = await prisma.$transaction(async (tx) => {
        ...
        // Same centralized ID service as manual creation — generated in-txn.
        const mid = await assignManexaId(tx, { userId: su.id, tenantId, kind: "STUDENT" });
      ...
      result.imported++;
      result.created.push({ name: r.name, type: "Student", manexaId, status: "Created" });
    } catch (e) {
      result.failed++;
