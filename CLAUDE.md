# MANEXA — project notes for Claude
...
## Architecture
- **Business logic lives in `src/lib/`** (service-shaped, never in route files): `engine.ts`, `tenancy.ts` (create/join institution + `MAN-XXX-######` IDs), `permissions.ts` (DB-driven RBAC `can()`), `automation.ts`, `auth.ts`, `audit.ts`, `notify.ts`, `face/*` (descriptor matching — `descriptor.ts` `cosineSimilarity` + `engine.ts`).
...
## Design system — READ `.claude/` BEFORE building any UI
The full, binding design rules live in **`.claude/`**: `design-system.md`,
